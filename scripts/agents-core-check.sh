...
#
# NO TOOLCHAIN. It is `grep`, `awk` and a SHA-256, so it runs in the
# ubuntu-latest `Shell scripts` job beside the other guards rather than needing
# a macOS runner -- which is also why it does not live in a `cargo` or
# `xcodebuild` step the way the sibling crates wire theirs.
set -euo pipefail
...
# sha256sum on linux, shasum -a 256 on macOS. This guard runs on ubuntu in CI
