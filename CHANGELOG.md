...
- **Home and About pages (v1.2.0).** The main window now opens on a new **Home** landing page, pinned to the top of the sidebar: a welcome header with the app version, live at-a-glance counts for mounted local volumes / configured network drives / empty drives, a showcase of every supported filesystem (ext4 and NTFS read-write; EROFS and SquashFS read-only; qcow2 / VHD / VHDX / VMDK disk-image containers; SMB / FTP / SFTP / WebDAV / S3 / Dropbox / Google Drive / OneDrive network and cloud), and Add Disk Image / Add Network Drive quick actions. Returning users land on Home automatically; genuine first-run still shows the folder-approval setup pane and switches to Home once a folder is approved (`ContentView` injects the initial selection from `HomeAccessService.hasFolder`). A new **About** page, pinned to the sidebar bottom, presents the project description, an architecture summary (FSKit-backed block devices, File Provider for network/cloud), the full vendored-library version table (name / version / commit / date, with a dirty badge), and the MIT licence + source link — the roomy home for the version information the cramped menu-bar About window used to carry. That menu-bar **About DiskJockey** window is now a compact standard about box (icon, version, build date) that simply points at the in-app About page. Two new views (`HomeView`, `AboutPageView`) join the synchronized app group with no project-file edits; the only `.pbxproj` change is the version bump.

- **EROFS & SquashFS volumes now auto-mount from real media.** Read-only EROFS/SquashFS partitions (e.g. RAUC A/B root slots on an MBR SD card) mount through their FSKit extensions. Three fixes: (1) `probeResource` did a sub-block read (96/128 bytes) where `FSBlockDeviceResource` requires **block-aligned** reads — it returned EINVAL and the probe silently failed; it now rounds the read up to a whole block. (2) The read-only extensions' `Info.plist` was missing `FSActivateOptionSyntax` (FSKit's signal that a module supports the mount/activate operation) plus `FSCheckOptionSyntax`, and the literal `"Linux"` (MBR 0x83) media-type hint alongside the GPT Linux GUID — so the system never offered them the slices nor let them activate. (3) `fskitd` caches its module→media-type registry, so it must be restarted (`scripts/dev.sh reset-daemons`) after a media-type change to re-offer matching media. ext4 was unaffected (already read block-aligned + declared both keys).
- **Home page shows live extension enable/disable state.** The Filesystems section reflects which FSKit extensions are actually switched on in System Settings — read (never written) **in-app via FSKit's public `FSClient` API** (`FSClient.shared.installedExtensions` → `FSModuleIdentity.isEnabled`). This is sandbox-safe and MAS-compatible with no helper process; it needs no extra entitlement. (`pluginkit`/`pkd` is sandbox-blocked, and an unsandboxed XPC agent can't ship through the Mac App Store, so `FSClient` is the right path.) Disk-image containers are shown as always-available; network/cloud follows the File Provider extension's state.
- **Honest filesystem detection.** `SwiftPartitionProbe` no longer falls back to *guessing* a filesystem (it used to assume ext4 for any Linux 0x83 slice, NTFS for 0x07, etc.); an unrecognized partition is reported `unknown` rather than mis-typed and mis-mounted. A `.probe` event no longer stands up a mislabeled `.mounting` sidebar row for a slice an extension merely looked at and rejected.
- **`DiskJockeyAgent` stays a dev-only helper (not shipped).** An unsandboxed XPC agent can't be registered by the sandboxed app via `SMAppService` (BTM requires a sandboxed target) and can't ship through the Mac App Store — so it is *not* embedded; it remains a developer convenience (`scripts/install-agent-dev.sh`) for disk-image probing. Its build was repaired along the way (`NSXPCConnection.setCodeSigningRequirement` replacing the non-public `auditToken`+SecCode check).

## 2026-06-02

- **Swift 6 strict-concurrency warnings cleared across the project.** Build went from 12 strict-concurrency warnings (all "this is an error in the Swift 6 language mode") to zero. Five files touched, each fix mechanically scoped to the exact warning site. `DiskJockeyEXT4/EXT4Backend.swift` (4 warnings — `OpaquePointer` Sendable unavailable on the lock's State + 3 non-Sendable closure captures of `UnsafeMutableRawPointer`/`UnsafeRawPointer`) gained three local `@unchecked Sendable` wrappers (`BridgeHandle` for the OpaquePointer state, `UncheckedMutableBuffer` + `UncheckedConstBuffer` for the FFI pointer params); 21 `state.withLock { fs in … guard let fs = fs else }` sites renamed to `{ handle in … guard let fs = handle.ptr else }`; the three pointer-passing sites wrap the param at function entry. `DiskJockeyLibrary/IOStats/IOStatsRecorder.swift` (1 warning — `snapshot` captured-var ref in concurrently-executing code) bound the post-preflush snapshot to an immutable `let snapshotToWrite` before passing it into the lock's write closure. `DiskJockeyApplication/Views/Mount/AddMountView.swift` (3 warnings — missing `@unknown default` on switches over `DirectMountScheme`) added the missing case with sensible fallbacks: `EmptyView()` for the `@ViewBuilder` switch, `return false` for the form-completion check, `throw POSIXError(.EINVAL)` for the submit-creation switch (lands in the existing catch block's `errorMessage`). `DiskJockeyApplication/Views/Mount/DirectMountDetailView.swift` (1 warning) added `@unknown default: EmptyView()` to the `StoredMountConfig` switch. `DiskJockeyApplication/Services/OAuth/OAuthLoopbackListener.swift` (3 warnings — `var resumed: Bool` captured + mutated across NWListener.stateUpdateHandler closures) converted to an `actor`: `bindResumed` is now actor-isolated state, NWConnection/NWListener callbacks bounce through `Task { await self.… }` to enter actor isolation; the once-only state-handler swap and the `respondAndClose` / `htmlPage` / `parseQuery` pure helpers become `private static func` (no actor hop needed). The class's 6 `listener.cancel()` call sites in `OAuthCoordinator.swift` gain `await` since `cancel()` is now actor-isolated. No behaviour change; build + 160 tests still green; the project is now ready for Swift 6 language mode without errors in these five files.
